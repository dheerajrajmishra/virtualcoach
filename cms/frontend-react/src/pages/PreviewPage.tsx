    ...
    <div className="p-8 max-w-2xl mx-auto">
      ...
      {ready.length === 0 ? (
        <div className="text-center py-16 text-gray-400 card">
          ...
          <Link to="/processing" className="btn-primary text-sm px-4 py-2">View Pipeline</Link>
        </div>
      ) : (
        <div className="space-y-3">
          {ready.map(t => (
            <button key={t.id} onClick={() => navigate(`/preview/${t.id}`)}
...
export default function PreviewPage() {
  ...
  if (!training) return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-gray-500">
      ...
      <button onClick={() => navigate(-1)} className="btn-primary text-sm px-4 py-2">Go back</button>
    </div>
  )
    ...
    <div className="flex flex-col h-full">
      ...
      {!hasSlides ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-3 text-gray-400 p-8">
          ...
          <Link to="/processing" className="btn-primary text-sm px-4 py-2 mt-2">View Pipeline</Link>
        </div>
      ) : (
